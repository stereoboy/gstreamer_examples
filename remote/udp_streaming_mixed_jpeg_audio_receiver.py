...
def build_video_pipeline(port):
    ...
    stereo_video_pipeline = Gst.parse_launch(
        f"udpsrc port={port} buffer-size={WIDTH*HEIGHT*3} caps=application/x-rtp,media=video,encoding-name=JPEG ! rtpjpegdepay !  queue ! jpegparse ! jpegdec ! textoverlay text='remote left' ! autovideosink \
        udpsrc port={port + 1} buffer-size={WIDTH*HEIGHT*3} caps=application/x-rtp,media=video,encoding-name=JPEG ! rtpjpegdepay !  queue ! jpegparse ! jpegdec ! textoverlay text='remote right' ! autovideosink"
        )
...
def main():
    ...
    parser.add_argument('-H', '--hostname', action='store', dest='hostname', default=DEFAULT_HOST, help='sender\'s hostname or ip address')
    ...
    stereo_video_pipeline = build_video_pipeline(options.port)

    audio_out_pipeline = build_audio_out_pipeline(options.port + 2)
    audio_in_pipeline = build_audio_in_pipeline(options.hostname, options.port + 3)

    pipelines = [stereo_video_pipeline, audio_out_pipeline, audio_in_pipeline,]
