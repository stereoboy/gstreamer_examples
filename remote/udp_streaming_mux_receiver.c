...
int main(int argc, char *argv[]) {
    ...
    data.rtpjpegdepay = gst_element_factory_make ("rtpjpegdepay", "rtpjpegdepay");
    ...
    if (!data.pipeline || !data.source || !data.rtpjpegdepay || !data.jpegdec || !data.sink) {
    ...
    gst_bin_add_many (GST_BIN (data.pipeline), data.source, data.rtpjpegdepay, data.jpegdec, data.sink, NULL);
    if (!gst_element_link_many (data.source, data.rtpjpegdepay, data.jpegdec, data.sink, NULL)) {
...
static void pad_added_handler (GstElement *src, GstPad *new_pad, CustomData *data) {
    ...
    GstPad *sink_pad = gst_element_get_static_pad (data->rtpjpegdepay, "sink");
